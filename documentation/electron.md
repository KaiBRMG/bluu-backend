...
> **`/api/app-update` takes an optional Bearer token, and answers either way.** It returns a public version number and the public `/download` URL — the same facts as the GitHub release. `appUpdateConfig.ts` remains the single gate; the route just delivers it to a client whose bundle is older than the deploy.
>
