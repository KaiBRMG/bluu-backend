...
| `electron/snip.html` | The **Snipping Tool** selection surface (requires build **0.13.0+**; the page and the sidebar both gate on it) — one **fully transparent** window per display. It draws nothing but the rectangle being dragged: the desktop stays live underneath and there is no scrim. See [snipping-tool.md](snipping-tool.md). |
| `electron/snip-preload.js` | Preload for `snip.html` **only** — commit/cancel, a microphone **status read** and a settings hand-off, and **no image or media channel by design**. There is deliberately no `requestMic`: a permission prompt would take focus, and on a single display that cancels the snip. Not the app preload: that window is invisible, always-on-top and watching the mouse. |
| `electron/snip-record.html` | The **snip recorder** (build **0.14.0+**): the control bar shown while a recording runs, and the page holding the `MediaRecorder`. Opened only once the selection surfaces are destroyed, and marked **content-protected** so it is not in the recording it controls. See [snipping-tool.md](snipping-tool.md). |
| `electron/snip-record-preload.js` | Preload for `snip-record.html` **only**. Chunks go **out**; nothing sends this page anything. The desktop stream lives here and never reaches the app window, which loads remote content. |
| `electron/snip-frame.html` | The **recording frame** — a click-through border marking the region being recorded, shown for the life of a recording. No script and **no preload at all**: it has nothing to say to main. Sized to the rectangle grown by 3px per side, so its ink sits outside the crop. |
...
| `gologin.launch(idToken, profileId, deviceId)` | invoke | `gologin:launch` | Runs the GoLogin SDK **in main** to open a profile in **Orbita — a separate application window, not an Electron one**. In order: claims the server-side session lock (refuses with `in-use` + the holder if someone else has it), fetches **the caller's own** API token from `/api/gologin/launch-token`, instruments the Orbita downloader, then starts. Heartbeats the lock every 60s. Idempotent per profile; capped at 5 live sessions. **v0.12.0+** |
| `gologin.stop(profileId)` | invoke | `gologin:stop` | Two-step teardown: kill Orbita, **then** commit the profile back to GoLogin. The SDK's `stop()` does NOT close the browser — see [gologin.md](gologin.md). Bounded, re-entrant, and shared with the manual-close watcher and the quit path |
...
| `gologin.orbitaStatus()` / `ensureOrbita(v)` | invoke | `gologin:orbita-status` · `gologin:ensure-orbita` | Orbita is a separate Chromium build (hundreds of MB) from GoLogin's CDN. Only `downloadBrowserArchive` is replaced on the SDK's checker — extraction and hash verification stay the SDK's. v0.12.0+ |
| `gologin.busyProfiles()` / `closeDecision(d)` | invoke | `gologin:busy-profiles` · `gologin:close-decision` | Profiles open or still uploading, and the answer to the guard dialog (`cancel` / `force` / `after-completion` / `stop-and-close`). v0.12.0+ |
...
| `snip.configure(config)` | invoke | `snip:configure` | arms/disarms the global shortcut + tray item. Main-window only, and **never cached to disk** — see [snipping-tool.md](snipping-tool.md). Returns `{ok, shortcutRegistered}`; `false` there means another app owns the combination. Also carries **`supportsRecording`**: capability negotiation running the opposite way to a version floor, so a new shell does not offer Video to an old page bundle that has no `onRecorded` listener |
...
| `snip.onRecorded(cb)` / `removeRecordedListeners()` | main→renderer | `snip:recorded` | a finished recording is waiting. Carries an opaque **token**, never a path — the bytes are in a temp file main owns. Feature-detect |
| `snip.uploadRecording(opts)` | invoke | `snip:uploadRecording` | **main streams the temp file to Cloud Storage** over the signed URL this window fetched. The one place in the app where main uploads; see [snipping-tool.md](snipping-tool.md) for the four reasons. Main-window only |
| `snip.discardRecording(token)` | invoke | `snip:discardRecording` | delete a queued recording for good — the slot was refused, or the user said so. A failed *transfer* keeps its file. Main-window only |
| `snip.listPendingRecordings()` | invoke | `snip:listPendingRecordings` | recordings still waiting to upload, failures included. The queue is a directory in `userData`, so this survives a quit, a crash and a reboot |
| `snip.onPendingChanged(cb)` / `removePendingListeners()` | main→renderer | `snip:pending-changed` | the queue changed; the page never polls |
| `snip.onUploadProgress(cb)` | main→renderer | `snip:upload-progress` | bytes **confirmed by the bucket** during a resumable upload, never bytes written to the socket |
| `snip.savePendingRecording(token)` | invoke | `snip:savePendingRecording` | the escape hatch — native save dialog + `copyFile` for a recording that will not upload. Does NOT clear the queue entry |
| `snip.onAudioPrefs(cb)` / `removeAudioPrefsListeners()` | main→renderer | `snip:audio-prefs` | the System audio toggle changed on the selection surface, which has no session. This window persists it |
...
**Three intakes, not two — and the third is easy to miss.** `second-instance` fires only when the app was **already running**. A link that *launches* a cold app on Windows/Linux puts the URL in the first process's `process.argv` and fires no event at all, so `deepLinkFromArgv(process.argv)` is read once at start-up into `deeplinkUrl` (v0.10.2+). Without it a cold launch opened the app on its last route and silently dropped the link. macOS never populates argv this way, so that read is inert there.

The cold path then relies on `pendingDeepLinkRoute`, not on the IPC event: the replay happens at the **splash screen's** `did-finish-load`, long before the app renderer exists, so the send lands nowhere and the parked payload is what `DeepLinkRouter` collects on mount.
...
> **`/api/app-update` takes an optional Bearer token, and answers either way.** It returns a public version number and the public `/update` URL — the same facts as the GitHub release. `appUpdateConfig.ts` remains the single gate; the route just delivers it to a client whose bundle is older than the deploy.
>
> A token is what lets the route resolve a **cohort** (below) to the caller: it reads their `groups` off the cached `users` doc and returns only what is targeted at them. Without a token — or with one that fails to verify, which is treated as no token rather than a 401 — it falls back to `allUsers` entries only. Locking a stale renderer out of the update policy is how a fleet gets stranded on a broken build, so this route never refuses to answer.

### Cohorts: staging a release

Each entry in `appUpdateConfig.ts` (`mac`, `win`, **and** `releaseNote`) carries an `UpdateCohort` — `allUsers` / `uids` / `groups` — matched exactly like [`announcementConfig.ts`](../src/lib/announcementConfig.ts) and `emailMigrationConfig.ts`: a user matches when `allUsers` is true, **or** their uid is listed, **or** one of their groups is. So a release can be piloted on one person, then a group, then the fleet, each step a one-line commit.

- **`allUsers: true` is the historical behaviour** and the right end state for a real release. `allUsers: false` with both lists empty targets **nobody** — inert, not a wildcard.
- **The match runs server-side**, in the route, and only the caller's own answer crosses the wire. A platform the user is not targeted by arrives as `null`, indistinguishable from a disarmed platform — the renderer never holds a list of which colleagues are targeted.
- **A cohort only narrows; it does not relax [rule 14](../CLAUDE.md#cross-cutting-rules-do-not-violate).** Arming a cohort of one before the artifacts exist still blocks that one person on a build they cannot obtain.
- **The offline fallback narrows too.** `fetchAppUpdateConfig()` falls back to the compiled constant *resolved against an unknown user* — `allUsers` entries only. The renderer cannot evaluate a cohort it was never sent, and falling back to the raw constant would prompt (or, on `compulsory`, block) every user who briefly lost the network, including the ones a pilot was deliberately keeping clear. For an `allUsers` release the fallback is exactly what it always was.
- **The release note's cohort is `AND`ed with its version gate**, and a user outside it is left **unmarked** rather than marked-as-notified — so widening the cohort later still reaches them. `POST /api/user/app-version` is the authority; `AppVersionReporter`'s client-side check is only a pre-check that stops a non-targeted user re-posting on every app start.
...
- `setPermissionRequestHandler` + `setPermissionCheckHandler` deny every renderer permission request except **`clipboard-sanitized-write`** (`ALLOWED_PERMISSIONS` in `main.js`); screen capture uses `desktopCapturer`, not `getUserMedia`, so it's unaffected.
  ...
  - **The camera remains denied, and so does the microphone for this window.** The Snipping Tool's narration (0.15.0) did not widen `media` here: the microphone is opened in the recorder window alone, gated on `snipRecording.audio.mic`. The app window loads the deployment, so voice messages would still need a native change — decide before shipping, not after.
