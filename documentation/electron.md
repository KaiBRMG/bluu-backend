...
| `electron/snip-preload.js` | Preload for `snip.html` **only** — commit/cancel, the microphone permission probe, and **no image or media channel by design**. Deliberately not the app preload: that window is invisible, always-on-top and watching the mouse. |
