...
3. **How** — `permissions:resetScreenCapture` in `main.js` runs `tccutil reset ScreenCapture com.bluu.app`. **darwin-only**; no root needed (verified); `execFile` (no shell). Capped at **once per OS user, ever** by a `.screencapture-tcc-reset-done` marker in `app.getPath('userData')`, written **before** `tccutil` so a crash mid-reset can't loop. `userData` **survives app updates/reinstalls**, and TCC is keyed per **OS-user + bundle id** (not per Bluu uid) — the correct granularity, and the reason the once-guard lives here rather than in the renderer flag (a second Bluu account on the same Mac must not re-reset an already-fixed record).
