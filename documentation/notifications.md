...
- **Users still in first-run onboarding are skipped.** A brand-new user has no "before" to compare the release against — the whole app is new to them — and the note would land in their tray while they are still granting OS permissions. `POST /api/user/app-version` checks `hasCompletedOnboarding` off the snapshot it already reads and, when it is not `true`, **stamps `releaseNoteNotifiedVersion` without sending**. Marking rather than skipping is the point: the note must not arrive late the moment they finish onboarding. It suppresses only the note armed at that moment — the next release reaches them normally.
