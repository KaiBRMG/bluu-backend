...
- **Never rendered during onboarding.** `AnnouncementCard` is mounted on `(main)/layout.tsx`, which is above the onboarding steps too, so it gates itself on `hasCompletedOnboarding` (and on the `/onboarding` path, covering the window between submitting the form and the flag landing in the snapshot). The gate suppresses the fetch as well as the render. A first-run user has not reached the app yet, and for the Telegram entry it would pre-empt onboarding's own Link Telegram section with the identical ask.
