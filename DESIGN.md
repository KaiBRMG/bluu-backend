...
- **The Windows certificate steps are visible, not collapsed.** Required setup behind a disclosure is how the previous version buried the one thing a Windows user must do. Repeat visitors are handled by a single "Updating? …skip to step 2" line, not by hiding the steps.
...
3. **Never quote a version number.** `/api/app-update` serves the *update-prompt policy*, cohort-gated and per-platform — not "what is in the Drive folder". Reading it here would print a claim the page cannot honour.
4. **Punctuation never trails a chip.** `UI_CHIP` / `FILE_CHIP` carry horizontal padding, so a comma or full stop immediately after one floats away from the sentence. Write the copy so a word follows the chip, or end the line on the chip.
