...
- **A detail inside a detail drills down in the same panel; it does not open a second one.** Two stacked Radix dialogs mean two overlays darkening the canvas twice, two focus traps, and an `Esc` that only dismisses the top one — paid for a panel that is entirely hidden behind the one in front of it. One panel with two levels keeps a single trap, and the way back out is the **same back control in the same place** as every other back control in the subsystem. Build it by splitting the inner detail's *body* out of its own `Sheet` wrapper, so one component serves both the standalone panel and the nested level, and an optional `onBack` is what tells it which it is — never a second copy of the content.
- **Rank a list of rows with a magnitude bar, not a chart.** A translucent Action Blue wash behind each row, widened in proportion to the best row in *that list*, is the house idiom ([`SalesReport`](src/components/salary/SalesReport.tsx)'s `By creator` list) and answers "which of these actually worked" before any number is read. Keep it translucent or it paints over the row's own hover wash and hovering goes patchy. It encodes the value and nothing else, so it does not breach the Semantic-Only Rule. **A row with no reading gets no bar at all** — a zero-width bar and "never measured" would look identical, which is the same gap-invention the next bullet bans.
