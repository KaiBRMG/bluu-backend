...
- **A detail this substantial is a page, not a sheet.** Once an item's detail carries a table with its own controls, it has outgrown a 512px panel. Make it a full-width view with a back control, held in **page state rather than a route** when the data is already in memory — switching costs no refetch and no app-shell remount (CLAUDE.md rule 9 and the app-shell known issue). The trade is that the view is not linkable; take it only where nothing is shared by URL. Every such view opens with the same back control in the same place, so leaving any of them is one gesture.
