...
- **Action Blue** (`#3b82f6`): The single interactive accent — tints, borders, selection washes, and marks that carry no text. Used sparingly; it is the one voice that means "act here."
  ...
  **It is a token now: `--action-blue` / `--action-blue-deep`, with `bg-action-blue`, `text-action-blue`, `border-action-blue` and `ring-action-blue` wired through `@theme`.** Prefer those to `[#3b82f6]`. The accent had been hand-typed as an arbitrary value at roughly two dozen call sites, which is how an accent forks without anyone deciding to fork it — and it made "change the accent" a find-and-replace across the app rather than one line. Existing `[#3b82f6]` spellings elsewhere in the app still work and are not wrong; convert them when you are already in the file.
...
### Growth Tracking category palette (authored divergence)
The Growth Tracking roster carries its **own** five-hue palette in [`src/lib/growth/category.ts`](src/lib/growth/category.ts) (`CATEGORY_TONE`) — one entry per `GrowthCategory` (TWXNK / BONUS / CREATOR / SFW REPOST / GENERAL), each a `{ chip, active, dot }` triad. Like [`stateColors.ts`](src/lib/stateColors.ts) above it is a **parallel system, not drift**: these hues mean *this operational grouping*, not "success" or "warning", and reading them out of the status palette would tie a roster label to a status change. **Import it; never re-map a category to a hex or a class inline.** A change to the status palette does not propagate here.

It is the closed-vocabulary case the [Don't-hash-a-string rule](#6-dos-and-donts) explicitly permits — the set is fixed in code, a value outside it is not stored, and the colours were specified with the grouping — which is also why the platform mark beside it stays greyscale.

**The filled (selected) step is measured per hue, not assumed.** `-600` is not one step that clears AA everywhere; it depends on the hue's luminance, and two of these shipped failing on the assumption that it did. White on `green-600` is **3.30:1** and on `orange-600` **3.54:1**, against a 4.5:1 floor for the chips' 12px non-bold text — both are now `-700` (5.02:1 and 5.18:1). `purple-600` (5.39:1), `blue-600` (5.17:1) and `zinc-600` (7.60:1) pass as they are. **Measure any hue added here against white before it ships**; the table lives in `category.ts`.

**Semantic hues needed as raw hexes** — an SVG `stroke`, a chart prop, anywhere a Tailwind class cannot reach — come from `STATUS_HEX` in [`campaignTracking.ts`](src/lib/campaignTracking.ts), the same `-400` steps `STATUS_COLORS` names. Reach for a class first; that map exists so nobody re-types `#4ade80` into a component.
...
- **Earnings** (600, `text-3xl` / 30px, `tabular-nums`, `tracking-tight`, `leading-none`): The **second** sanctioned step above `Display`, and the last — one agent's own pay, on the CA salary surfaces only ([`SalarySummaryCard`](src/components/salary/SalarySummaryCard.tsx) and the `Overview` headline in [`dashboard/salary/page.tsx`](src/app/(main)/ca-portal/dashboard/salary/page.tsx)). It earns the step on the same grounds as `Instrument`: this figure *is* those surfaces' reason to exist — the question that used to require asking someone for a spreadsheet — and it is the one number on the screen a person has a material stake in. **One size, both places.** It previously ran `text-3xl` on the card and `text-4xl` on the page, which is the failure this step exists to prevent: the same number at two sizes reads as hero-metric instinct rather than a decision. Like `Instrument`, do **not** cite this step to justify a third big number; everywhere else `Display` remains the ceiling.
...
**A ring on an avatar is a status, never a decoration.** A creator's face is the densest identifier the product has, and in a ~90px calendar cell it is the *only* one — so the one modifier that survives there is a ring. `CreatorChip`'s `overtime` prop draws `ring-2 ring-orange-400` (status-orange, which already means overtime/cover across CA Portal) with a `ring-offset` against the surface so adjacent faces in a dense row stay separable. Two conditions on using it: the meaning must appear as **visible text beside the row** (`+2 OT`, `2 overtime`) — the Semantic-Only Rule means a ring nobody can decode is a coloured pixel encoding nothing — and the ringed items sort **last** in a truncating list, so an overflow `+N` hides the exception rather than the norm.

**The ring states the category, never its consequence.** `CreatorChip`'s hover is the account's name and nothing else. A marked account means "overtime", but whether overtime is *paid* depends on the rest of the shift — a fact the chip cannot see — and the title asserted "no extra pay" for both cases, which was false on a shift where every account is overtime and therefore paid. A ring is worn by one item; the sentence explaining it belongs to the row, which knows the context. This generalises: **if a per-item marker has to explain itself, it is explaining something the item does not know.**
...
- **The overlay recipe has one definition: `SURFACE` in [`src/lib/surfaces.ts`](src/lib/surfaces.ts)** (`border border-white/[0.07] bg-white/[0.025]`), with `HAIRLINE` for a bare divider and `SURFACE_INTERACTIVE` for a panel that is itself a control (hover/active overlay steps plus the standard focus ring). Compose it — `cn('rounded-xl p-5', SURFACE)` — and keep radius and padding at the call site: a panel and a popover header are the same *material* at different sizes, and a component taking both as props is a `div` with a worse name. These two values were hand-typed at roughly two dozen sites on the salary and shift surfaces alone, and the same "card" had already drifted to `0.02` and `0.03` in neighbouring elements with nothing recording which was intended.
...
#### The satellite-window shell (OF Manager, GoLogin)

`AppLayout` is the shell for every **in-app page**. The one sanctioned exception is a surface that owns its own Electron window — `/of-manager` and `/gologin` — where a sidebar and top bar would be navigation to nowhere. Its chrome is the two-pane messaging shell instead:
...
**GoLogin uses a single-pane variant.** `/gologin`'s profile console has no second entity to justify a right pane, so it renders only the left rail's chrome — eyebrow section title, hairline rules, the overlay recipe, filter chips — without the two-pane split. A future single-purpose satellite window follows GoLogin's variant; a window with a genuine second entity (a thread + its context, a list + its detail) follows OF Manager's two-pane form.
...
**A second instance: the employee registry** (`/admin-portal/user-management`). Same rail, same two-line rows, same crossfading action lane — it is the shape a roster of people wants, and it replaced a card grid. Two additions the pattern did not have before, both reusable:

- **Sections are a derived closed vocabulary, not an alphabet.** `userStage()` ([`userStatus.ts`](src/components/admin/user-management/userStatus.ts)) collapses three raw fields into `Not set up` / `Active` / `No access` / `Archived`, each **borrowing its hue from `STATUS_COLORS`** exactly as `disputeStatus.ts` does. The `NOT SET UP` rail sits first, which is how a chase queue lives *inside* an index instead of becoming its own destination. Derive a stage this way rather than sectioning on a raw enum.
- **Selection is a second target in the leading slot, and it is not permanent chrome.** The avatar crossfades to a checkbox on `:hover`, on `:focus-within`, and whenever a selection is already under way; a bulk action bar then appears on `--z-banner`. A resting list stays a list of *people* rather than a column of controls, and the keyboard path is the same reveal the action lane already uses. Selecting and opening are different intents, so they are allowed to be two targets — the "one row, one target" rule is about two hit areas pointing at the *same* destination.

### The decision queue (Disputes)

The CA dashboard's **Sale Disputes** column and the All-disputes dialog behind it are the reference for a **queue of decisions** — a list whose task is "rule on each of these", as opposed to the faceted index's "find the one I need" or a table's "audit the column". When the reader's job on every row is *approve or reject*, build this shape rather than a table with an actions column. *(This lived at `/ca-portal/disputes` until 2026-09-18; that route is now a redirect onto the dashboard. Everything below is unchanged by the move.)*
...
- **A reviewer who rules the same way on many rows gets a selection, not a repetition.** The admin table (`ca-portal/admin` → Disputes) carries a leading checkbox column and a bulk bar above it; the agent-facing queue does not, because ruling on one sale at a time is that page's whole job. Four rules make the bar safe: it **reserves its height at zero selection** (a bar that appears on the first tick shoves the row the reviewer was aiming at out from under the cursor); selection is **derived against the rows currently on screen**, so a page or filter change cannot carry an invisible id into the next action; a selected row shows **two cues** — the checked box and the Action Blue tint — with hover left on the neutral overlay so hue separates the two states; and the bar is offered **only where every row takes the same two verdicts**, never on a list whose actions vary per row. The checked box needs the explicit Action Blue override (§2's `--primary` issue), or selection reads as disabled.
...
- **A queue narrow enough to sit beside the page has a second, compact form — and it is a row, not a card.** [`SaleDisputesPanel`](src/components/disputes/SaleDisputesPanel.tsx) runs the same queue in a 23rem column: two lines (amount + creator chip; filer + short date) and an `icon-sm` tick/cross lane. Four rules make the compression honest rather than lossy:
  - **It rests on the panel at the list-item overlay step** (`bg-white/[0.04]` → `hover:0.055`), never with a border of its own. A bordered card inside a bordered panel is a nested card, and at 23rem the two frames eat most of the width they enclose.
  - **The whole row opens the full record**, through an `inset-0` overlay button with the verdict lane `relative` above it. Making the text block a button *beside* the lane leaves a dead strip under the buttons and another in the gap, on a row that is 46px tall. The detail dialog carries the fan, the exact time in the reader's zone, the reviewer and the argument in full — the four facts the column drops — and it offers the **same** two verdicts on the same terms, because a reviewer who opened a dispute to read the argument should not be sent back out to press a 28px tick.
  - **The compact form never becomes the only form.** The column shows the first four and links the remainder into the dialog by count (`3 more waiting`); it does not paginate, filter or sort. A column that grew those controls would be the page again, in a third of the width.
  - **The reason bar is shared, not re-implemented** — one [`RejectReasonBar`](src/components/disputes/disputeUi.tsx) with a `layout="stack"` variant for the column. Three copies is how one of them loses `Esc`.
- **A queue also has to report what happened to *your* filings, and the word carries the outcome, not the hue.** `Approved — moving to your report` / `Rejected by an admin`, at `text-[11px]` in the matching `-400` ink, dismissible for 14 days into `localStorage`. Same pattern and same reasoning as `LeaveBalanceCard`'s decision pills: green and red at 11px on a dimmed screen is not a distinction someone should have to make about whether they were paid.

### The metric roster (Growth Tracking)

`/smm-portal/smm-growth-tracking` is the reference for a **roster of things each carrying its own trend** — a collection whose task is "which of these is moving, and which one do I open". Reach for it when every item has a headline number and a history, and the items differ enough in magnitude that one shared axis would flatten most of them into the baseline. It is the third shape beside the faceted index ("find the one I need") and the decision queue ("rule on each of these").

- **Per-item scale beats a shared axis.** Give each item its own sparkline on its own scale rather than overlaying every series on one chart. A field of lines needs a re-basing mode to stay readable, turns into a rainbow the moment you colour it, and degrades further as the collection grows; a card is legible at any magnitude, and the headline figure beside it is what states the magnitude. Comparison then belongs to the ranked figures and to an interrupt band (below), not to overlapping traces.
- **The whole card is the button.** A card with no other interactive element inside it *is* the control — the "one row, one target" rule, applied to a grid. (This is the opposite call from a table row, where `role="button"` on a `<tr>` overrides `row`, orphans the cells and takes the sortable headers' column associations with them, and where a `box-shadow` focus ring is never painted under `border-collapse: collapse`.) Cards sit on the overlay recipe, so they take the overlay hover steps rather than a `brightness` nudge.
- **Grid, not a fixed column count.** `repeat(auto-fill, minmax(min(100%, 250px), 1fr))`. The `min(100%, …)` is what stops a card overflowing its container on a narrow window.
- **A standing stat row is range-independent on purpose.** The tiles above the roster answer "how big is this operation" and are read *before* a filter is chosen; if their meaning changed with the chips below them, the same glance would mean something different every time. Filters and ranges belong to the collection, not to its summary. Do not sum across units that are not the same unit — two totals are more useful than one meaningless one. **A tile may name a thing rather than count one** ("Biggest Mover", "Fastest Growing"), and when it does the Display-step figure stays on the *metric* with the identity on the Meta line beneath it — a row whose big text is sometimes a name and sometimes a number does not scan as a row. Derive it from the same function the surfaces below use, so a tile and the collection can never disagree.
- **An interrupt band earns a hue and no motion.** A strip above the roster naming the few items worth acting on right now takes the *attention needed* tint (`bg-orange-500/[0.06]`, `border-orange-500/20`) and an orange dot — **static**. A pulse looping forever on a console people sit in front of for eight hours trains them to stop seeing it, and a gradient carrying no state is exactly what the Semantic-Only Rule forbids. The band is loud relative to the greyscale grid below it, which is all it needs to be.
- **When a threshold decides whether a band appears at all, put the threshold on the band.** An empty band and an over-full one are then fixed in the same glance instead of hunting for a setting, and the band must render its own empty state rather than vanishing — a control whose surface disappears as you drag it reads as broken. State the resulting count beside it.
- **The band ignores the collection's filters, deliberately.** Its job is to interrupt with something the reader was *not* already looking at; one that only reported inside the current filter could never do that. Any badge repeating the band's signal on a card must use the **same** threshold, so the two never disagree.
- **The detail is a panel, and the thing that forces it wider is a component, not an amount of content.** This bullet previously read "a detail this substantial is a page, not a sheet", on the grounds that an item's detail had grown a five-column sortable table and a table does not fit 512px. That was the wrong conclusion drawn from a true observation: what had outgrown the panel was **the table**, not the detail. Replacing it with a strip per row — one row, one line of text, one figure, one sparkline — put the whole detail back inside a panel, and bought back the thing a full-page detail costs and cannot return: **the collection stays on screen behind it**, so opening an item is a peek rather than a departure and moving between items does not bounce through an overview you never left. So: reach for a panel first, and when something inside it genuinely cannot fit, redesign *that component* at panel width before you promote the whole detail to a page. Go full-width only for a detail whose content is irreducibly wide — a real matrix, a diff, a timeline that must be read across.
- **A detail inside a detail opens in place, not in a second panel — and not as a second level of the first.** Two stacked Radix dialogs mean two overlays darkening the canvas twice, two focus traps, and an `Esc` that only dismisses the top one. Replacing the panel's contents instead avoids all three and is still wrong for a detail that belongs to a list: **the list is the context for every number in it**, so a detail that covers it turns comparing two items into a round trip with nothing on screen in between. Expand the item's own card (`Accordion type="single" collapsible`, so one is open at a time and the surface never becomes a page of stacked detail), and keep the item's summary in the header while it is open — repeating the opening line is the accordion convention, it holds the header at a predictable height instead of reflowing everything below on each open, and it keeps the item's own text selectable outside the trigger button. Reserve the full panel for an item that arrived *alone*, where there is no list to preserve and the width buys something the card cannot hold.
- **Show all of a small metric set instead of a picker that reveals one.** A segmented control above a list, re-keying every row to one chosen metric, is the reflex — and it is the wrong trade whenever the whole set fits on one line of the row. Five figures on a 16px line cost less space than the control does, and they remove a decision the reader did not want to make. Reach for the picker only when the set is too large or too wide to show at once. **When the numbers come from an outside platform, borrow that platform's own glyphs and their order** — the reader already has that layout memorised, so the figures land without a label being read. Each glyph is then `aria-hidden` with the metric named in `sr-only` text, because a heart means nothing read aloud; and a metric the source reports inconsistently renders `—`, never `0`.
- **A control that appears to change nothing has picked the wrong target.** If a window, filter or scope both *selects* the items and *measures* them, check whether the two cancel out: a 7-day window over a list already cut to the last 7 days makes "change over 7 days" identical to each item's lifetime total, and the control reads as broken. Pick one job — usually measurement, since it keeps the collection whole and comparable — and let the figures move.
- **A trend mark on a cumulative measure must draw the rate, not the total.** The "figure + delta + sparkline" idiom assumes a series that can go both ways. On a measure that only ever rises — cumulative engagement, lifetime spend, total sessions — a trace of the total is the same shape on every item that ever worked, and a sparkline normalised to its own min/max makes +3 and +1,600 draw an identical full-height climb: the card's largest band then contradicts its own headline. Draw the per-interval **rate** instead, zero-based, so the mark rises while the thing is happening and decays to nothing when it stops — and state that rate's current value beside the line, in words, so the mark is not decoration. Divide by the interval rather than plotting raw increments whenever the sampling is uneven, or the longest gap draws as a spike. This is the shape half of the test the colour bullet above applies; run both.
- **An expandable card's open state must outrank every hover state, in a cue hover cannot imitate.** Hover washes composite on top of the item's own ground, so a rest of `0.025` plus a hover of `0.03` renders brighter than an "open" step of `0.04` — the open card ends up the dimmest thing in the list, and identical `hover:` / `open:` border values leave nothing to break the tie. Raise the open background above the composited hover ceiling **and** give open the Action Blue edge: an open card is the current selection, which is precisely the job the accent exists for.
- **An item card and the detail it opens are one component, and the expansion carries no chart.** The card's own sparkline is the trend; the expansion states the record — every reading, with the moment it was taken. A charting instance per open card in a scrolling list is precisely the cost a hand-drawn sparkline exists to avoid, and the log is the same truth in the form that survives at that size.
- **Reuse a card's construction one level down, but re-decide its colour.** A nested collection (an item's children) should be read with the same eye movement as the parent roster — same bands, same order, same figure-then-shape rhythm — so the reader learns one card, not two. **The hue does not transfer automatically.** The roster's green/red works because a follower count genuinely falls; carried onto a *cumulative* measure it is green on every card every day, and a hue that never varies encodes nothing, which is what the Semantic-Only Rule exists to stop. Spend the nested card's colour on whatever actually varies at that level instead, and leave the trace greyscale. Ask what the hue distinguishes before you copy it down.
- **Never invent a value for a gap.** A missing reading is absent, not zero: charting a zero draws a cliff to the axis and reads as a collapse that never happened. A delta with fewer than two readings renders `—`, not `0` — "we have not measured this" and "this did not change" are different facts, and a `0` meaning the first is a lie the reader cannot detect. A single-point sparkline draws a dashed hairline rather than a flat line implying a measurement.

### The shaded matrix (CA Admin → Overview)

`ca-portal/admin` → Overview is the reference for **one measure across two entities at once** — here gross revenue by agent *and* by creator. Reach for it when the interesting fact is the intersection rather than either total, and when both margins are small enough to stay on one screen (roughly a dozen rows by a dozen columns; past that, fold the tail into an `N more` column and rank it in a list underneath).

- **One scale for the whole grid, never per row.** Normalising each row to its own maximum makes every row's best cell look identical and destroys the down-the-column read — which is the read a matrix exists for. State the scale in the section's meta line so nobody has to infer it.
- **Shade the cell; do not bar it.** The app's magnitude idiom elsewhere is a bar behind the row ([the sales report's `By creator` list](src/components/salary/SalesReport.tsx)), and it stays right for any row wide enough to hold one. A matrix column is ~80px, where a proportional bar becomes a two-pixel sliver sitting beside a right-aligned number — an encoding nobody reads and a stray mark beside every figure. A tint carries the same ordering at any cell size. **This does not breach the Semantic-Only Rule**: the only thing the tint encodes is the value. Ramp it with `color-mix(in oklab, var(--action-blue) N%, transparent)` — the token, not a hex — from a small floor (so the smallest real figure still reads as present) to well short of opaque (so white text keeps its contrast on the darkest cell). Keep the ramp translucent, or the cell's own background paints over the row's hover wash and hovering goes patchy.
- **Absent and zero are different, and must look different.** No intersection at all renders `—` on an untinted cell; a real figure that nets to zero renders the figure. Inventing a zero for a gap is the same lie the metric roster warns about.
- **The row-label column is sticky.** The grid scrolls sideways at the 1024px window floor, and a matrix whose row labels scroll away is unreadable. A sticky cell needs an opaque ground to cover the cells passing under it, which means it cannot *also* show the row's translucent hover wash — paint that wash as a `before:` overlay on the sticky cell and lift its content with `relative`.
- **Compact figures in the grid, exact figures at the edges.** `formatUsdCompact` keeps ten columns on screen but is explicitly not for anything anyone reconciles, so the cell carries the exact amount in its `title` and `aria-label`, and the two columns people actually reconcile against — the row total and the ranked list below — are never rounded.
...
- **Focus is one ring, and it comes from `--ring`.** `focus-visible:ring-[3px] focus-visible:ring-ring/50`, plus `focus-visible:ring-inset` on rows, facets and any full-width element so the ring can't be clipped by the row's own rounding. This is exactly what every shadcn primitive already draws, so a hand-rolled control and a `Button` focus identically — before this they did not, and a single tab pass through a page changed the indicator's colour *and* its width depending on which kind of control you landed on.

  **`--ring` is `#93c5fd`, a light tint of Action Blue — not Action Blue itself, and the reason is arithmetic.** The 50% opacity is baked into 18 shadcn primitives, so the ring has to clear WCAG 1.4.11's 3:1 floor *after* that blend. On this app's near-black ground, stock shadcn's grey measures **1.99:1** (it failed on every control in the product), Action Blue at `/50` measures **2.15:1**, and `#93c5fd` measures **3.44:1** on the canvas and **3.19:1** on the sidebar. Keyboard focus still reads as the accent voice — it is the same hue — it is just light enough to survive being halved. Never remove the ring without replacing it with something equally visible, and never re-ink a focus ring in-component.
...
- **Activity spinners keep spinning under `prefers-reduced-motion`.** The global block in `globals.css` freezes every animation on the page, so a spinner covering a real wait must claim the exemption or it reads as a hung app. `.loader` and `.network-modal-spinner` are whole spinner components; **`.activity-spinner` is a bare marker** for the inline lucide `Loader2Icon` in a button (a live scraper call, a save). Never `motion-reduce:animate-none` on a spinner — that opts *into* the freeze, which is backwards.
- **A ribbon draws in as one span, not as segments popping independently.** [`SessionWalkthroughDialog`](src/components/timesheet/SessionWalkthroughDialog.tsx)'s state ribbon (`.session-ribbon-draw`, 260ms ease-out, `scaleX` from a left `transform-origin`) wraps every segment in one element so the whole bar reads as a single span of time running forward on open, the same reasoning as the notification tray's beats: convey the one thing the object *is* (a duration), not decorate its arrival.
...
- **Don't** introduce a second font family, oversized display type (the only exceptions are the time-tracking `Instrument` step and the CA-salary `Earnings` step — see § Typography), gradient text/heros, or glassmorphism — this is not a marketing site.
...
**The skin is defined once, in [`src/app/model-submissions/_lib/theme.ts`](src/app/model-submissions/_lib/theme.ts).** Import those tokens; never inline a hex on this surface. The azure triad and `PANEL` now live in [`src/lib/publicSkin.ts`](src/lib/publicSkin.ts) and are **re-exported** from that file — they moved the moment §9 needed them, because the alternative was a third literal `#00b8f5` with nothing recording which spelling was intended. The import path here is unchanged. Full subsystem detail — including the abuse model — lives in [model-submissions.md](documentation/model-submissions.md).
...
1. **Import the skin from `_lib/theme.ts`.** It lives under `src/app/download/` and `/update` imports it from there — the skin has one home, not a copy per route. New colour → add it there **and** to this section.
...
## 9. Installer page (external skin) — "the lit bench"

The **fourth** surface: `/download` **and `/update`**, public and unauthenticated (both allowlisted in [`middleware.ts`](src/middleware.ts)), and the one place in the product whose entire job is a decision someone can get wrong. Like §7 and §8 this is an **authored divergence**, not drift.

**One skin, two routes, one difference.** `/update` is `/download` with the certificate sequence and the walkthrough removed — everything below applies to both unless a bullet says otherwise. They are separate files on purpose: the shared half is tokens, not markup, and a `?mode=` on one page would put the two readers back in the same component with a flag deciding between them, which is exactly the guess this split exists to delete.

**The skin is defined once, in [`src/app/download/_lib/theme.ts`](src/app/download/_lib/theme.ts)**, which imports the shared azure triad and `PANEL` from [`publicSkin.ts`](src/lib/publicSkin.ts). Import those tokens; never inline a hex on this surface.

### Who is reading it
Two people, and they owe nothing to each other — **which is why they now get a route each.**
- **A new colleague** on a machine with no app on it → **`/download`**. Their job is the choice, so the choice is the page, and the certificate is step 1 of a numbered sequence.
- **A Windows user reinstalling for a release** → **`/update`**, where `APP_UPDATE.downloadUrl` points. They already trust the certificate and want the `.exe` in one click, so the certificate is not a step here at all — it is one line linking back to `/download`, for the one reader who can still arrive wrongly: someone who followed an update link onto a machine that has never had the app. **macOS is never sent to `/update` by the update prompt** (it updates in-app); a Mac arrives at either page only by being handed the link.

**The route is the answer, not a guess.** `/download` still infers its reader from a `localStorage` marker (below), and that marker is erased by a new browser, cleared site data or a private window — so the inference fails toward showing the certificate, which is the safe way to be wrong on a *first-install* page and the wrong way to be wrong on an update. Splitting the routes is what removes the guess from the update path entirely. **`/update` is therefore stateless: no marker, no branching, every reader sees the same page. Do not add one.**

### What carries over
Near-black ground, `PANEL` overlay surfaces, hairline borders, no drop shadows, azure as the one voice, `AZURE_INK` on any azure fill, shadcn primitives only, tabler/lucide icons only.

### What differs (the authored part)
- **It is a task surface, not a brand surface.** The type scale sits between the console's and §8's — `h1` at a fixed `2rem` → `sm:2.5rem`, body at 15–16px, a wider `860px` measure — and **nothing animates on arrival**. §8's louder display scale is not inherited.
- **The wash is placed, not ambient.** `BENCH_GROUND` puts the single azure glow *behind the install panel* rather than above a masthead: the panel is the reason the page exists. One wash, same as every other surface gets exactly one. **Do not add a second.**
- **The platform picker is a `Tabs`, preselected from the user agent.** Read through `useSyncExternalStore` (server snapshot `null`), never a `setState` in an effect, so SSR and hydration agree. An explicit pick always wins over detection. A Mac preselects **Apple Silicon** deliberately: the x64 `.dmg` carries no arch suffix, so a Silicon user who takes it runs under Rosetta and keeps taking x64 updates from then on — that failure has a long tail and the reverse does not. It is what "Not sure which Mac you have?" is for, and why it sits beside the button.
- **`OPTION` marks selection with three signals** — border, fill and the azure icon plus a check — never hue alone (WCAG 1.4.11). Its `!` overrides are load-bearing: shadcn's `TabsTrigger` ships its own `dark:data-[state=active]:` rules at specificity 0,2,0.
- **`/download` only — on Windows the certificate is step 1, with its own download button, above the `.exe` button.** The `.exe` will not run until the certificate is trusted, so the panel is a numbered sequence and **step 1 holds the filled button until the certificate has been taken** — the unfilled step-2 button stays live and legible beside it (`STEP_BTN_QUIET`), never disabled. Taking the certificate moves the fill to step 2, so the page follows the reader rather than describing the order. `StepMark` carries three states — done, current, upcoming — because an upcoming step wearing the current step's fill is the page contradicting its own sequence.
- **`/download` only — first visit vs. revisit is a `localStorage` marker**, read through `useSyncExternalStore` and **frozen for the session**, so the layout cannot rearrange itself mid-read. It records two facts, not one: that the page was *seen*, and that the certificate was actually *downloaded here*. **Only the second demotes step 1** — someone who looked once and left has not done the crucial step, and demoting it for them is the exact failure this flow prevents. A returning reader gets the `.exe` as the single filled action with the certificate folded to one line; it is never hidden, and "First time installing Bluu Backend?" reopens the full sequence — the question is phrased around the *install*, not the machine, so a reader who is only updating can rule themselves out without opening it.
- **Written to be skimmed.** Everything that is not the instruction, the file, or the decision was cut — no positioning paragraph, no detection commentary, no restating what the selected tab already says.
- **A `<noscript>` block carries all three installers.** The picker is a client control; without it a Mac visitor would be stranded on the Windows default. On `/download` it also carries the certificate; on `/update` it carries the link to `/download` instead.

### Surface rules (in addition to §1–6)
1. **Import the skin from `_lib/theme.ts`.** It lives under `src/app/download/` and `/update` imports it from there — the skin has one home, not a copy per route. New colour → add it there **and** to this section.
2. **Say only what is true of the files.** This page is read by someone who cannot yet reach any part of the product, so a wrong instruction here has no in-app correction. The `@bluurock.com` sign-in notice was removed in 2026-09 when staff moved onto personal Google accounts — it had become a false barrier. **Do not restate an access requirement here without confirming it.**
3. **The version on the buttons comes from [`latestRelease.ts`](src/lib/latestRelease.ts), never from `appUpdateConfig.ts`.** Two reasons, both load-bearing. `APP_UPDATE.latestVersion` is a *policy* — cohort-gated, per-platform, `null` for Windows most of the time — and this page needs the *fact* about what is in the Drive folders. And importing `appUpdateConfig.ts` into this client component would bundle `APP_UPDATE`'s `uids`/`groups` cohort lists into a page served to anyone with the link. **Bump `LATEST_RELEASE_VERSION` when the files land in the folders**, per cross-cutting rule 14 — a number the folder cannot honour sends someone looking for a file that is not there. The certificate button carries no version; the certificate is not versioned. **Both routes read the same constant** — a version that disagrees between `/download` and `/update` is the same file described two ways.
4. **Punctuation never trails a chip, and a chip never wraps.** `UI_CHIP` / `FILE_CHIP` carry horizontal padding, so a comma or full stop immediately after one floats away from the sentence — write the copy so a word follows the chip, or end the line on it. Both chips are `whitespace-nowrap`: a chip broken across two lines paints as two half-chips.
