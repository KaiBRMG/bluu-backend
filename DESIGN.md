...
The **fourth** surface: `/download`, public and unauthenticated (allowlisted in [`middleware.ts`](src/middleware.ts)), and the one page in the product whose entire job is a decision someone can get wrong. Like §7 and §8 this is an **authored divergence**, not drift.
...
### Who is reading it
Two people, and the page owes them different things:
- **A new colleague** on a machine with no app on it. Their job is the choice, so the choice is the page.
- **A Windows user reinstalling for an update** — `APP_UPDATE.downloadUrl` points here. They already trust the certificate and want the `.exe` in one click. **macOS is never sent to this page by the update prompt** (it updates in-app); a Mac only arrives here for a first install.
...
- **On Windows the certificate is step 1, with its own download button, above the `.exe` button.** The `.exe` will not run until the certificate is trusted, so the panel is a numbered sequence and **step 1 holds the filled button until the certificate has been taken** — the unfilled step-2 button stays live and legible beside it (`STEP_BTN_QUIET`), never disabled. Taking the certificate moves the fill to step 2, so the page follows the reader rather than describing the order. `StepMark` carries three states — done, current, upcoming — because an upcoming step wearing the current step's fill is the page contradicting its own sequence.
- **First visit vs. revisit is a `localStorage` marker**, read through `useSyncExternalStore` and **frozen for the session**, so the layout cannot rearrange itself mid-read. It records two facts, not one: that the page was *seen*, and that the certificate was actually *downloaded here*. **Only the second demotes step 1** — someone who looked once and left has not done the crucial step, and demoting it for them is the exact failure this flow prevents. A returning reader gets the `.exe` as the single filled action with the certificate folded to one line; it is never hidden, and "Installing on a new machine?" reopens the full sequence.
...
- **A `<noscript>` block carries all three installers.** The picker is a client control; without it a Mac visitor would be stranded on the Windows default.
