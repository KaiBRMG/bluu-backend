...
9d. **NEVER call the Apify API yourself, and treat its inputs as billing decisions.** Same hazard as 9b: every actor run is billed per result. Do not `curl` `api.apify.com` to explore a payload or check a field name — use the actor pages, or ask the user. In code, the X actor's `getFollowers`/`getFollowing`/`getRetweeters` are the **$0.016-per-query** paths and are pinned `false` as assertions, `maxItems` is pinned to the batch size, and there is **one run per platform per night**. The nightly bill is ~$2.35/month; each of those three lines is what keeps it there. See [growth-tracking.md](documentation/growth-tracking.md).
