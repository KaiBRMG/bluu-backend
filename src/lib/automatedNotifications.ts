...
export const AUTOMATED_NOTIFICATIONS: AutomatedNotification[] = [
  ...
  },
  {
    ...
    trigger: 'The assigned CA approves the dispute, passing it up to a team leader. Coalesced — decisions made within a few minutes of each other arrive as one message naming the count.',
  ...
  },
  {
    ...
    trigger: 'The assigned CA rejects the dispute. Coalesced into one message per filer; the REASON clause is omitted when no reason is given, and lists every distinct reason when a batch collected more than one.',
    recipients: 'The user who submitted the dispute',
    sources: ['src/app/api/disputes/[disputeId]/ca-approval/route.ts'],
  ...
  },
  {
    ...
    trigger: 'A team leader gives final approval, one dispute at a time or on a selection from the admin table. Coalesced — one message per filer naming the count, not one per dispute.',
    recipients: 'The user who submitted the dispute',
    sources: [
  ...
  },
  {
    ...
    trigger: 'A team leader rejects the dispute, one at a time or on a selection from the admin table. Coalesced into one message per filer; the REASON clause is omitted when no reason is given.',
    recipients: 'The user who submitted the dispute',
    sources: [
      'src/app/api/disputes/[disputeId]/admin-approval/route.ts',
      'src/app/api/disputes/bulk-approval/route.ts',
