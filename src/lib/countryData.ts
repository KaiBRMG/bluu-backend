...
/**
 * Countries and their international dial codes, sorted by name.
 *
 * The list was originally ~60 entries covering the markets we happened to think
 * of, which is a reach problem rather than a data one: this array backs the
 * public application form's country **and** WhatsApp fields, so a country
 * missing here is an applicant who cannot finish the form. It is now the full
 * ISO-3166-1 set of places with their own dial code.
 *
 * `name` is stored as-is on user and submission records, so **renaming an entry
 * orphans the values already saved against it**. Add freely; rename only with a
 * migration.
 */
export const countryCodes: CountryCode[] = [
  ...
  { code: 'VG', dialCode: '+1', name: 'Virgin Islands (British)' },
  { code: 'VI', dialCode: '+1', name: 'Virgin Islands (U.S.)' },
...
/**
 * First country carrying this dial code.
 *
 * Ambiguous by nature — `+1` is the whole North American Numbering Plan and `+7`
 * covers Russia and Kazakhstan — so this resolves a *representative* country,
 * never "the" country. Where the specific country matters, key off `code`.
 */
