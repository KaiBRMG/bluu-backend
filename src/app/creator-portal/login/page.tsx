...
export default function CreatorLoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // CreatorAuthWrapper in layout will handle redirect once auth state updates
    } catch (err: unknown) {
      const code = (err as { code?: string })?.code;
      console.error('[creator-portal/login] auth error:', code);
      if (code === 'auth/invalid-credential' || code === 'auth/user-not-found' || code === 'auth/wrong-password' || code === 'auth/invalid-login-credentials') {
        ...
        setError(`Login failed. (${code ?? 'unknown'})`);
      }
    } finally {
      setLoading(false);
    }
